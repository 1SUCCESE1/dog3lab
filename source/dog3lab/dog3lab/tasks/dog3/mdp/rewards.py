...
def track_lin_vel_y_lateral_exp(
    ...
    - When a lateral command is active (|cmd_y| > threshold): reward y-velocity
      tracking ONLY when exactly one foot is on the ground (single-leg support)
      so the feet alternate lifting -- prevents gaming lateral tracking by just
      tilting the body with both wheels down.
    ...
    single_support = n_feet_in_contact == 1  # exactly one foot down
...
def wheel_action_rate(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    ...
    the wheels back-and-forth (accelerate -> reverse to brake -> accelerate
    again), which oscillates the trunk pitch. Only the wheel actions are used.
    """
...
def leg_activity(
    ...
    cmd = env.command_manager.get_command(command_name)
    cmd_x = cmd[:, 0]
    desired = -lean_gain * cmd_x  # forward cmd -> feet behind body
    err = torch.abs(x_mean - desired)

    # reward the swing matching the command direction; gate on command magnitude
    cmd_mag = torch.linalg.norm(cmd[:, :3], dim=1)
    moving_gate = torch.sigmoid((cmd_mag - command_threshold) / 0.05)
    return moving_gate * torch.exp(-err / 0.05)
...
def pitch_rate_l2(
    ...
    """Penalize base pitch angular velocity squared (damping of the balance
    channel -- the pitch-rate term of the LQR state).
    """
    asset = env.scene[asset_cfg.name]
...
def pitch_command_alignment(
    ...
    the trunk flat regardless of command by penalizing pitch that is aligned
    with the command sign, i.e. pitch*cmd > 0. The trunk should stay level
    ...
    # aligned = pitch * sign(cmd_x); penalize only when positive (tilt with cmd)
...
def lateral_leg_lift(
    ...
    """Reward actually lifting the legs when a lateral (y-axis) velocity command is given.

    Sideways movement of a wheeled biped needs the legs to lift off and step
    sideways (hip lateral swing), rather than just leaning the body. This
    reward is gated on |cmd_y| so the policy is incentivized to pick up a foot
    while strafing.

    Uses the foot's *position* above the terrain (not contact force) to detect
    a genuine lift: a foot whose z is at least ``lift_height`` above the ground
    counts as airborne. Contact-force-based detection (net_force < 1N) lets the
    policy game the reward by momentarily unloading the wheel without actually
    lifting it.
    """
    asset = env.scene[asset_cfg.name]
    # foot world z minus terrain z (env_origins) = height above ground
    foot_z = asset.data.body_pos_w[:, asset_cfg.body_ids, 2]
    ground_z = env.scene.env_origins[:, 2].unsqueeze(1)
    lift = torch.sum((foot_z - ground_z > lift_height).float(), dim=1)
    cmd = env.command_manager.get_command(command_name)
    lateral_gate = torch.sigmoid((torch.abs(cmd[:, 1]) - lift_threshold) / 0.05)
    return lateral_gate * lift
...
def reward_body_pos_to_feet_y(
    env: ManagerBasedRLEnv,
    asset_cfg: SceneEntityCfg,
    sigma: float
) -> torch.Tensor:
    """Penalise the farthest foot displaced laterally beyond sigma.

    ``sigma`` = dead-zone half-width in metres (5 cm for D1H, which has no
    lateral velocity). Footprints inside the dead-zone are free; beyond it
    the excess is penalised per foot. Returns magnitude (>0 = bad), pair
    with a negative weight in the config.
    """
    asset = env.scene[asset_cfg.name]
    foot_body_names = ["FL_foot", "FR_foot"]
    foot_ids = [asset.body_names.index(name) for name in foot_body_names]
    ...
    y_abs = torch.abs(rel_b[:, :, 1])
    y_max = y_abs.max(dim=1).values
    excess = (y_max - sigma).clamp(min=0.0)
    return excess
